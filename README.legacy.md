Сделано на основе https://github.com/itgalaxy/bmp2image

Требуется в версии до PHP7.2, но так как у нас минимальная версия - 7.4 - код не требуется

Представляет интерес как пример работы с форматами файлов.

```php
public static function imagecreatefrombmp($filename, $maxWidth = 65535, $maxHeight = 65535)
    {
        ...
        if (!($fh = @fopen($filename, 'rb'))) {
            throw new ImageResizeException(__CLASS__ . " ERROR: Can't retrieve file.");
        }
